...
function buildInstructionBlock(): string {
  return [
    ...
    "",
    "Before broad discovery for a non-trivial repo task, follow this exact protocol:",
    "",
    "1. Call the MCP tool `llm_mem.context_map` with `{ \"task\": \"<user task>\", \"workingDirectory\": \"<current repo/worktree root>\", \"maxCandidates\": 8 }` before broad file search.",
