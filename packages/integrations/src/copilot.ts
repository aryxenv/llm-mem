...
function buildSkillContent(): string {
  return [
    "---",
    "name: llm-mem",
    "description: Exact protocol for using llm-mem context packs before repo-specific coding, debugging, explaining, refactoring, or testing tasks.",
    "---",
    "",
    "# llm-mem",
    "",
    ...
    "",
    "Use llm-mem as a local context optimization layer. It is not a replacement assistant and it should not cause broad exploratory tool use before context is compiled.",
    "",
    "## Required first move",
    "",
    "For any repo-specific code edit, debugging task, refactor, test task, explanation, or architecture question, call the MCP tool `llm_mem.context_pack` before reading lots of files.",
    "",
    ...
    "```",
    "",
    "Do not supply `repoId` unless it is already known. The MCP server resolves the current repository from `workingDirectory` or the configured server root.",
    "",
    "## How to use the context pack",
    "",
    "1. Read the returned JSON.",
    "2. Start with `sections` whose `type` is `retrieved_context`, `constraints`, `diagnostics`, or `open_questions`.",
    "3. Treat `citations` and each section's `sourceRefs` as the trusted map of files and facts.",
    "4. Open cited files first when code needs inspection.",
    "5. Prefer the smallest edit path that satisfies the task and cited constraints.",
    "6. If tests or validation commands are cited, use those before inventing new validation.",
    "",
    "## If the pack is insufficient",
    "",
    "Do not immediately scan the whole repo. First call `llm_mem.context_pack` again with a narrower task or constraints, for example:",
    "",
    "```json",
    "{",
    ...
    "```",
    "",
    "Only fall back to normal file search after the narrower context pack still lacks the needed evidence.",
    "",
    ...
    "## Do not",
    "",
    "- Do not run shell `llm-mem context` when the MCP tool is available.",
    "- Do not read broad directories before the first context pack.",
...
function buildInstructionBlock(): string {
  return [
    ...
    "",
    "Before making code changes for a user task, follow this exact protocol:",
    "",
    "1. Call the MCP tool `llm_mem.context_pack` with `{ \"task\": \"<user task>\", \"workingDirectory\": \"<current repo/worktree root>\", \"maxTokens\": 8000 }` before broad file search.",
    "2. Read the returned JSON and use `sections`, `citations`, and `sourceRefs` as the trusted map of relevant files, constraints, diagnostics, and open questions.",
    "3. Open cited files first. Prefer the smallest edit path that satisfies the task and cited constraints.",
    "4. If the pack is insufficient, call `llm_mem.context_pack` again with a narrower task or `constraints` before scanning unrelated directories.",
    "5. Use `llm_mem.remember` only for durable source-grounded facts when `repoId` is available; never guess `repoId`.",
    "6. Do not run shell `llm-mem context` when the MCP tool is available, and do not paste the full pack to the user unless asked.",
    "",
